...
def opcuaClient():
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            TempVar = root.get_child(["0:Objects", f"{idx}:{NODE_NAME}", f"{idx}:{TEMP_VARIABLE}"])
            TempObj = root.get_child(["0:Objects", f"{idx}:{NODE_NAME}"])
            ...
            PressVar = root.get_child(["0:Objects", f"{idx}:{NODE_NAME}", f"{idx}:{PRESS_VARIABLE}"])
            PressObj = root.get_child(["0:Objects", f"{idx}:{NODE_NAME}"])
            ...
            TimeObj = root.get_child(["0:Objects", f"{idx}:{NODE_NAME}"])
